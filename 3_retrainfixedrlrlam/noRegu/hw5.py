...
def train_func(R, size, r,lam, lr):
    ...
    while loss > 0.01:
        ...
        for i in range(len(R)//batch): 
            ...
            W_spr_new = W_spr + lro * (P.transpose().dot(V_spr))
            V_spr_new = V_spr + lro * (P.dot(W_spr))
            ...
            if i % 5 == 0:
                ...
                scipy.sparse.save_npz('VW_100000/V_spr_15_0002'+'_'+str(lr), V_spr, compressed=True)
                scipy.sparse.save_npz('VW_100000/W_spr_15_0002'+'_'+str(lr), W_spr, compressed=True)
...
for lr in [0.001]:
    p = multiprocessing.Process(target = train_func, args = (train_mat,size,15,0.2,lr))        
    p.start()
p.join()
