    ...
    while loss > 0.01:
        ...
        for i in range(len(R)//batch): 
            ...
            if i % 2 == 0:
                ...
                scipy.sparse.save_npz('VW_100000/V_spr_15'+'_'+str(lr)+'_'+str(lamm), V_spr, compressed=True)
                scipy.sparse.save_npz('VW_100000/W_spr_15'+'_'+str(lr)+'_'+str(lamm), W_spr, compressed=True)
...
for lr in [0.001,0.0001]:
    for lam in [20.0,0.2,0.002]:
        p = multiprocessing.Process(target = train_func, args = (train_mat,size,15,lam,lr))        
        p.start()
p.join()
